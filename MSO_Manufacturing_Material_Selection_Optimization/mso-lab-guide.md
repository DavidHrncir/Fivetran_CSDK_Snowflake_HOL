# Fivetran Connector SDK Hands on Lab with Snowflake 2025: Manufacturing Material Supply Optimization
...
1. Create the project directory. On a Windows or Mac, create the **mso** folder in a folder that does not reside under a system folder such as "My Documents".

2. Download the project files from the git repo (easiest way):
   - **mso_manual.env**: Environment variables loading file
...
3. Once the above files are downloaded copy/move them to the newly created **mso** folder
...
6. Set up our environment variables.
   - Edit the **mso_manual.env** file.  These values will become environment variables in your current terminal session (meaning if you close the terminal by accident, you will need to re-apply these environment variables). NOTE: If these environment variables are not present, you will be prompted to enter them manually during debugging and deployment (added complexity).  Note that the **FIVETRAN_CONFIGURATION** and **FIVETRAN_PYTHON_VERSION** values are already set.  We need to update the first 3 values now.
      ...
      - **FIVETRAN_CONNECTION_NAME**: This must be all lower case letters, numbers, and underscores.  Since we are sharing a common environment, the <ins>connection names must be unique</ins>.  An option that works well here is, your initials followed by your birth month and day followed by mso.  Example: **dh0816mso** This value will be the connector name in Fivetran as well as the schema name in Snowflake.  Ensure you save the **mso_manual.env** file.
...
8. Copy all 5 values from the mso_manual.env file and paste them into your terminal and press enter/return.
...
***PAUSE: Here is where you would iterate and refine.***
...
6. Configure your app:
   - App title: **[your initials]-MaterialMind**
   - Database: Select **HOL_DATABASE_1**
   - Schema: Select your schema which is the same name as your connector (example: dh0816mso) the schema created by your Fivetran connector.
